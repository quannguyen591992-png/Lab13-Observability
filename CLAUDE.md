...
- Backend: Python FastAPI app (`app/main.py`) với Starlette middleware.
- Frontend: Không có frontend production; dashboard lab dùng local/simple evidence từ `/metrics` hoặc tool ngoài do nhóm chọn.
- Database: Không có database; state metrics/incident đang in-memory, logs ghi JSONL vào `data/`.
- Cache/Queue: Không dùng cache/queue.
- AI/LLM: Mock RAG + deterministic fake LLM trong `app/mock_rag.py`, `app/mock_llm.py`; tracing qua Langfuse nếu `.env` có key.
- Observability: `structlog` JSON logs, correlation ID middleware, PII scrubber, Langfuse decorators, in-memory metrics endpoint.
...
- Test: `pytest -q`
- Validate logs: `python scripts/validate_logs.py`
- Load test: `python scripts/load_test.py --concurrency 5`
- Inject incident: `python scripts/inject_incident.py --scenario rag_slow`
- Disable incident: `python scripts/inject_incident.py --scenario rag_slow --disable`
- Build: Không có build step riêng cho repo Python này.
- Lint/Format: Chưa cấu hình ruff/black trong repo; nếu thêm thì cập nhật lệnh tại đây.

### 5.3 Ghi chú riêng của dự án
- Không commit `.env`; anh tự điền Langfuse credentials vào `.env` từ `.env.example`.
- Trước khi chạy `scripts/validate_logs.py`, nên tạo log mới bằng cách chạy app và gửi request qua `/chat`; log cũ có thể làm score thấp nếu chứa record từ trước khi sửa.
- Passing criteria chính: `VALIDATE_LOGS_SCORE >= 80/100`, ít nhất 10 traces live trên Langfuse, dashboard đủ 6 panels, PII redaction pass, report `docs/blueprint-template.md` đầy đủ.
- Bonus audit logs ghi vào `data/audit.jsonl` khi bật/tắt incident endpoints.
